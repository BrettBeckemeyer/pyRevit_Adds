...
class PrintLinkedSheets(forms.WPFWindow):
    ...
    def _print_sheets(self, combined=False):
        ...
        if combined:
            ...
            print(str(len(self.linkedsheets_lb.SelectedItems)))
            for sheet in self.linkedsheets_lb.SelectedItems:
                #print_mgr.PrintToFileName = 'c:\Temp\z.pdf'
				#print(sheet)
                #pathh = '{}.pdf'.format(sheet.Name)
                #print(sheet.SheetNumber)
                #pathy = op.join('c:\Temp',pathh)
                #print(pathy)
                #print_mgr.PrintToFileName = op.join(USER_DESKTOP,'{}'.format(sheet.Name))
                print_mgr.PrintToFileName = \
                   op.join(savepath, '{} - {}.pdf'.format(sheet.SheetNumber,sheet.Name))
